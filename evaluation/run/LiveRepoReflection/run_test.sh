...
# "LiveRepoReflection-dataset"


# EVALUATION OUTPUT DIR
OUTPUT_DIR="LiveRepoReflection/evaluation/evaluation-output/LiveRepoReflection"
# OPENAI API BASE URL
...
BENCHMARK_PYTHON_SCRIPT_PATH="benchmark/benchmark_LiveRepoReflection.py"
...
TEST_SCRIPT_DIR="run/LiveRepoReflection/test.sh"
...
cd ${RUN_TEST_DIR} && bash ${TEST_SCRIPT_DIR} ${MODEL_NAME} ${OUTPUT_DIR} ${CUSTOM_OPENAI_API_BASE} ${CUSTOM_OPENAI_API_KEY} ${THREADS_NUM} ${MODEL_MAX_LEN} ${BENCHMARK_PYTHON_SCRIPT_PATH} ${PROXY_MODE} ${HACK_PROXY} ${AIDER_PARAMS}
